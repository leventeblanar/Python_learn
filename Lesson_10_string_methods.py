
name = input("Enter your full name: ")
phone_number = input("Enter your phone number: ")

# result = len(name) - length of string
# result = name.find(" ") - returns the first occurrance of given character
# result = name.rfind("o") - last occurrance of given character

# if the method cannot locate given character it gives -1

# name = name.capitalize() - capitalize first letter
# name = name.upper() - capitalize all 
# name = name.lower() - all lower case characters
# name.isdigit() - is the variable consist of all digits - return boolean
# restult = name.isalpha() - return boolean depending if variable only consists of alphabetical char

result = phone_number.count("-")


print(result)